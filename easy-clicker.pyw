...
class ClickAction:
    def __init__(self, position, button="left", duration=300, delay=300, number=1, double_click=False):
...
class EasyClicker:
    ...
    def create_clicker(self, button, double_click=False):
        number = len(self.actions) + 1
        action = ClickAction((100, 100), button, number=number, double_click=double_click)
    ...
    def close_all_clickers(self):
        # Gửi lệnh thoát cho tất cả clicker
        for number in list(self.clicker_processes.keys()):
            self.send_exit_command(int(number))
        
        # Chờ một chút để các process thoát
        time.sleep(0.1)
        
        # Terminate nếu chưa thoát
    ...
    def send_exit_command(self, number):
        """Gửi lệnh thoát cho một clicker cụ thể"""
        try:
    ...
    def start_clicking(self):
        ...
        # Đóng tất cả clicker và đợi 0.5s
        ...
        if filename:
            ...
            for action_data in config["actions"]:
                action = ClickAction(
                    ...
                    action_data["delay"],
                    action_data["number"],
                    action_data.get("double_click", False)
                )
                self.actions.append(action)
                self.recreate_clicker(action)
